...
namespace naps2_net80_1
{
    internal class Program
    {
        static async Task Main(string[] args)
        {
            ...
            //List all discovered scanner devices
            ...
            else
            {
                ...
                var targets = new[] { "fi-7160", "fi-8170" };
                var selectedDevice = devices.FirstOrDefault(d =>
                    targets.Any(t => d.Name.Contains(t, StringComparison.OrdinalIgnoreCase)));

                if (selectedDevice == null)
                {
                    Console.WriteLine($"No device found matching targets: {string.Join(", ", targets)}");
                    return;
                }   
                Console.WriteLine($"Selected device: {selectedDevice.Name}");
                await Scanner.ScanPdf(selectedDevice.Name, @"C:\temp\ScannedImages\scan.pdf");
